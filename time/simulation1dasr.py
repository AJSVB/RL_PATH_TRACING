...
class CustomEnv(gym.Env):
  ...
  def __init__(self, env_config):
    ...
    self.observation_space = spaces.Box(low=-1.0001, high=1.0001, shape=
                    (9,int(self.HEIGHT),int(self.WIDTH)), dtype=np.float32) #MACHINE PRECISION
    ...
    with open("comp/"+str(self.spp)+'msesdasr.txt', 'w') as fp:
        fp.write("\n")
    with open("comp/"+str(self.spp)+'psnrsdasr.txt', 'w') as fp:
        fp.write("\n")
  ...
  def reset(self):
    ...
    if self.bool:
     with open("comp/"+str(self.spp)+'msesdasr.txt', 'a') as fp:
         fp.write("\n".join(str(item.item()) for item in self.mses))
         fp.write("\n")
     with open("comp/"+str(self.spp)+'psnrsdasr.txt', 'a') as fp:
