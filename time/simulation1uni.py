...
#            loss+=self.criterion(self.denoised-self.olddenoised,self.gd.unsqueeze(0)-self.oldgd.unsqueeze(0))
          ...
          self.state = torch.clip(self.state.detach(),0,1)
...
class CustomEnv(gym.Env):
  ...
  def __init__(self, env_config):
    ...
    with open("images/"+str(self.spp)+'mses1.txt', 'w') as fp:
        fp.write("\n")
    with open("images/"+str(self.spp)+'psnrs1.txt', 'w') as fp:
        fp.write("\n")
  ...
  def reset(self):
    ...
    if self.bool:
     with open("images/"+str(self.spp)+'mses1.txt', 'a') as fp:
         fp.write("\n".join(str(item.item()) for item in self.mses))
         fp.write("\n")
     with open("images/"+str(self.spp)+'psnrs1.txt', 'a') as fp:
